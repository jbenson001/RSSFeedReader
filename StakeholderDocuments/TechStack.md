...
- Configure CORS to allow only the known frontend origin, not `*`.
- Store API base URLs and runtime settings in configuration files, not in hard-coded strings.
- Use dependency injection and interfaces for backend services so business logic stays testable and replaceable.
- Keep controllers thin by moving subscription and feed logic into service classes.
- When adding feed parsing, use `System.ServiceModel.Syndication` for safe XML handling and set HTTP timeouts.
- Render feed items using safe text display; do not inject or render raw HTML from remote content.
- Use strong typing for request/responses and avoid dynamic JSON shapes.
- Add unit tests for backend service logic and frontend component behavior when possible.
