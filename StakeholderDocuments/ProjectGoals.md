...
## Quality, security, and maintainability principles

- Implement the subscription feature with clean abstractions so the MVP can be extended without refactoring the whole app.
- Keep UI state handling separate from backend business logic and API communication.
- Validate feed URLs in a consistent way and reject clearly invalid input before adding a subscription.
- Design the backend API with small, strongly typed request/response models that support future persistence and feed operations.
- Protect the app by using scoped CORS rules, not wildcards, and by avoiding unsafe rendering of feed content.
- Use consistent naming, small methods, and clear comments for non-obvious logic.
- Prefer compile-time checked models and services so the code is easy to maintain and refactor later.
