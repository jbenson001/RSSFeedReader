...
- Validate that the input is a non-empty URL before creating a subscription entry.
- Keep the subscription management flow simple but structured: UI components call a backend service, not raw data stores.
- Use clear error messages if the add-subscription API call fails, even in MVP.
- Keep API models small and explicit so the backend can later evolve to persistence or fetch logic without breaking clients.
- For Extended-MVP feed display, do not render raw HTML from feeds; display only safe text values such as title and link.
- Build with a service-oriented design to allow replacing the in-memory store with a database later.
